...
int wi_connect(const char * device_id, char ** to_device_id,
  ...
  for (int nRetry = 0; nRetry < RETRY_TIMES; nRetry++) {
	  if (lockdownd_client_new_with_handshake(phone, &client, "TAWebkitProxy.exe") != LOCKDOWN_E_SUCCESS) {
		  printf("[webinspector][wi_connect][lockdownd_client_new_with_handshake] - time: %d - FAILED\n", nRetry);
		  sleep(1);
	  }
	  else {
		  printf("[webinspector][wi_connect][lockdownd_client_new_with_handshake] - time: %d - SUCCEEDED\n", nRetry);
		  break;
	  }
  }
  ...
  if (to_device_id)
  {
	  plist_t uniqueDeviceIdNode = plist_dict_get_item(node, "UniqueDeviceID");
	  plist_get_string_val(uniqueDeviceIdNode, to_device_id);
	  plist_free(node);
	  node = NULL;
  }
  ...
  if (to_device_name)
  {
	  plist_t deviceNameNode = plist_dict_get_item(node, "DeviceName");
	  plist_get_string_val(deviceNameNode, to_device_name);
  }
