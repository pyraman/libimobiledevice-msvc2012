...
dl_status iwdp_on_attach(dl_t dl, const char *device_id, int device_num) {
  ...
  char* ios_version = NULL;
  char* szMacAddr = NULL;
  char* szSerialNo = NULL;
  ...
  std::string szDeviceName = device_name;
  Utility::tolower(szDeviceName);

  std::string szpProductModel = product_model;
  Utility::tolower(szpProductModel);
  //add device info into cache
  DeviceInfo *deviceInfo = new DeviceInfo(
	  device_num,
	  LOCAL_HOST,
	  iwi->iport->port,
	  szDeviceName.c_str(),
	  szpProductModel.c_str(),
	  MANUFACTURER,
	  product_model,
	  ios_version,
	  szMacAddr,
...
ws_status iwdp_on_check_proxy_status(ws_t ws, bool is_head, bool want_json) {
	ws_status ret;
	if (DeviceManager::getInstance()->hasError()) {
		ret = iwdp_send_http(ws, is_head, "200 OK", ".html", "not_ready");
	}
	else {
		ret = iwdp_send_http(ws, is_head, "200 OK", ".html", "alive");
	}
	return ret;
...
ws_status iwdp_on_http_request(ws_t ws,
  ...
  if (strlen(resource) && !strncmp(resource, REQUEST_IS_DEVICE_BUSY, strlen(REQUEST_IS_DEVICE_BUSY)))
  {
	  const int nMaxLength = 1025;
	  char decodedResource[nMaxLength] = { 0 };
	  Utility::decode(resource, decodedResource);

	  bool isBusy = DeviceManager::getInstance()->isDeviceBusy(decodedResource);

	  return iwdp_send_http(ws, is_head, "200 OK", ".html", isBusy ? "0" : "-1");
  }
  else if (strlen(resource) && !strncmp(resource, REQUEST_CLEANUP_DEVICE, strlen(REQUEST_CLEANUP_DEVICE)))
  {
	  const int nMaxLength = 1025;
	  char decodedResource[nMaxLength] = { 0 };
	  Utility::decode(resource, decodedResource);

	  DeviceManager::getInstance()->cleanUp(decodedResource);
	  return iwdp_send_http(ws, is_head, "200 OK", ".html", "0");
  }
  ...
  if (is_websocket) {
	...
	else if (!strncmp(resource, "/closeproxy/", 12)) {
		ws_status ret = iwdp_send_http(ws, is_head, "200 OK", ".html", "close proxy");
		DeviceManager::getInstance()->backupData();
	...
	else if (!strncmp(resource, "/proxystatus/", 13))
	{
		return iwdp_on_check_proxy_status(ws, is_head, false);
