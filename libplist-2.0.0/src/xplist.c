...
static size_t dtostr(char *buf, size_t bufsize, double realval)
{
    ...
    f = modf(f, &ip);
    len = _snprintf(buf, bufsize, "%llu", ((f < 0) && (ip >= 0)) ? "-" : "", (int64_t)ip);
    if (len >= bufsize) {
        return 0;
    }
                ...
                if (!strncmp(entp, "amp", 3)) {
                    ...
                    *(entp-1) = '>';
                    ...
                    if (*(entp+1) == 'x' || *(entp+1) == 'X') {
                        ...
                        val = _strtoui64(entp+2, &ep, 16);
                    } else {
                        if (entlen < 2) {
                            PLIST_XML_ERR("Invalid numerical character reference encountered, sequence too short: &%.*s;\n", entlen, entp);
                            return -1;
                        }
                        val = _strtoui64(entp+1, &ep, 10);
                    }
    ...
    while (ctx->pos < ctx->end && !ctx->err) {
        ...
        if (*(ctx->pos) == '?') {
            ...
            if (!strcmp(tag, "plist")) {
                ...
                struct node_path_item * path_item = (node_path_item *)malloc(sizeof(struct node_path_item));
                if (!path_item) {
                ...
                struct node_path_item * path_item = (node_path_item *)node_path;
                node_path = (node_path_item *)(node_path->prev);
                free(path_item);
            ...
            if (!strcmp(tag, XPLIST_DICT)) {
                ...
                if (!is_empty) {
                    ...
                    if (tp->begin) {
                        ...
                        data->intval = _strtoui64((char*)str, NULL, 0);
...
err_out:
    ...
    while (node_path) {
        struct node_path_item *path_item = node_path;
        node_path = (node_path_item *)(path_item->prev);
        free(path_item);
    }
