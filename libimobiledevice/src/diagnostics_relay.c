...
LIBIMOBILEDEVICE_API diagnostics_relay_error_t diagnostics_relay_query_ioregistry_entry(diagnostics_relay_client_t client, const char* name, const char* klazz, plist_t* result)
{
	if (!client || (name == NULL && klazz == NULL) || result == NULL)
		return DIAGNOSTICS_RELAY_E_INVALID_ARG;
	...
	if (klazz)
		plist_dict_set_item(dict,"EntryClass", plist_new_string(klazz));
